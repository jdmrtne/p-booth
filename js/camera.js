...
function checkAndShowLandscapePrompt() {
  // Landscape prompt is now only shown when the camera session begins — see beginSession()
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Landscape prompt is only shown when the camera is opened — not on page load/orientation change
...
function buildThumbnailGrid() {
  ...
  // ── Desktop side-panel: match strip column layout exactly ──
  thumbnailsGrid.className = `thumbnails-grid ${cols > 1 ? 'cols-2' : 'cols-1'}`;
  ...
  // ── Mobile start-card thumbs: ALL shots in correct grid ──
  if (mobileThumbs) {
    mobileThumbs.innerHTML = '';
    mobileThumbs.style.display = 'grid';
    mobileThumbs.style.gridTemplateColumns = `repeat(${cols}, 1fr)`;
    mobileThumbs.style.gap = '4px';

    for (let i = 0; i < layoutConfig.shots; i++) {
      const s = document.createElement('div');
      s.style.cssText = 'width:44px;height:44px;background:linear-gradient(135deg,#fef0f6,#f4efff);border:2px dashed rgba(168,85,247,.2);border-radius:8px;overflow:hidden;position:relative;flex-shrink:0;';
      s.id = `mob-thumb-${i}`;
      const num = document.createElement('div');
      num.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:.58rem;font-weight:800;color:rgba(168,85,247,.4);pointer-events:none;';
      num.textContent = String(i + 1).padStart(2, '0');
      s.appendChild(num);
      const img = document.createElement('img');
      img.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:0;transition:opacity .3s;';
      img.id = `mob-img-${i}`;
      s.appendChild(img);
...
function buildUploadSlots() {
  ...
  for (let i = 0; i < layoutConfig.shots; i++) {
    ...
    slot.draggable     = !touchMode;

    // Both desktop & mobile get the drag handle — CSS makes it pink+large on touch devices
    slot.innerHTML = `
      ...
      <input type="file" accept="image/*" ${touchMode ? 'capture="environment"' : ''} style="display:none;" id="upload-file-${i}" onchange="handleSingleUpload(event,${i})">
    `;
    ...
    // ── Touch drag via handle — works on mobile AND hybrid desktop ──
    // Non-passive touchstart calls preventDefault() BEFORE browser locks scroll mode
    ...
    // ── Long-press fallback on slot body (180ms) — catches iOS scroll-swallowing ──
    {
      let lpTimer = null, lpActive = false, lpDragging = false;
      ...
      slot.addEventListener('touchstart', e => {
        ...
        lpActive = true; lpDragging = false;
        lpTimer = setTimeout(() => {
          if (!lpActive) return;
          lpDragging = true; isDragging = true; dragSrcIndex = i;
          slot.classList.add('dragging');
      ...
      slot.addEventListener('touchmove', e => {
        ...
        if (!lpDragging && (dx > 8 || dy > 8)) { clearTimeout(lpTimer); lpActive = false; return; }
      ...
      slot.addEventListener('touchcancel', () => {
        ...
        slot.classList.remove('dragging');
        document.querySelectorAll('.upload-slot').forEach(s => s.classList.remove('drag-over'));
        lpDragging = false; isDragging = false; dragSrcIndex = null;
      }, { passive: true });
    }
...
async function beginSession() {
  ...
  // On mobile we handle orientation programmatically via lockLandscape/lockPortrait —
  // no manual-rotate overlay needed any more.
...
function updateThumbnail(i, dataUrl) {
  ...
  // ── Mobile start-card thumb ──
  const mobSlot = document.getElementById(`mob-thumb-${i}`);
  const mobImg  = document.getElementById(`mob-img-${i}`);
  if (mobSlot) {
    mobSlot.style.borderStyle = 'solid';
    mobSlot.style.borderColor = layoutConfig.accentColor + '88';
    // Hide the number label once a photo fills the slot
    const numEl = mobSlot.querySelector('div');
...
async function openFullscreenCamera() {
  ...
  // Hide landscape overlay (legacy — no longer shown, but guard just in case)
  const lsOverlay = document.getElementById('landscapeOverlay');
  if (lsOverlay) lsOverlay.classList.remove('visible');

  // ── ORIENTATION: switch to landscape for camera capture ──
  document.body.classList.add('fsc-camera-active');
  if (typeof window._pgCheckOrientation === 'function') window._pgCheckOrientation();
  if (typeof window.lockLandscape === 'function') await window.lockLandscape();
...
function fscCloseOverlay() {
  ...
  // ── ORIENTATION: return to portrait after camera closes ──
  document.body.classList.remove('fsc-camera-active');
  if (typeof window._pgCheckOrientation === 'function') window._pgCheckOrientation();
  if (typeof window.lockPortrait === 'function') {
    window.lockPortrait();
  }
...
async function fscStartBurst() {
  ...
  // ── GUARD: block shooting in portrait mode ──
  // (Device API orientation check — fallback to window dimensions)
  const inPortrait = screen.orientation
    ? screen.orientation.type.startsWith('portrait')
    : window.innerHeight > window.innerWidth;

  if (inPortrait && isMobileDevice()) {
    // Flash the progress label as a warning
    const warn = document.getElementById('fscProgressLabel');
    if (warn) {
      const prev = warn.textContent;
      warn.textContent = '📱 Rotate to landscape to shoot!';
      warn.style.color = '#ff6b9d';
      setTimeout(() => { warn.textContent = prev; warn.style.color = ''; }, 2200);
    }
    return;
  }
...
// ── THUMBNAIL STRIP ─────────────────────────────────────────────────
// Shows ALL shots arranged in the same column/row structure as the final strip.
function fscBuildThumbs() {
  ...
  const cols = layoutConfig.cols || 1;
  const totalShots = layoutConfig.shots;

  // Size thumbnails so the whole grid fits comfortably in the side panel.
  // For tall layouts (>4 rows) shrink slightly.
  const rows = Math.ceil(totalShots / cols);
  const thumbSize = rows <= 2 ? 52 : rows <= 4 ? 44 : 36;

  // Override CSS to use grid so columns are respected
  strip.style.display = 'grid';
  strip.style.gridTemplateColumns = `repeat(${cols}, ${thumbSize}px)`;
  strip.style.gap = '4px';
  strip.style.alignContent = 'start';
